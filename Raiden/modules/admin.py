...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton(
            "ᴅᴇᴍᴏᴛᴇ", callback_data="demote_nobara({})".format(user_member.user.id))
    ]])     
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup([[
            InlineKeyboardButton(
              "ᴘʀᴏᴍᴏᴛᴇ", callback_data="promote_nobara({})".format(user_member.user.id))
       ]]) 
...
def refresh_admin(update, _):
    ...
    keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton(
            "ᴀᴅᴍɪɴ ᴄᴀᴄʜᴇ", callback_data="admin_nobara({})".format(user_member.user.id))
    ]])
            ...
            msg.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "ᴍᴇssᴀɢᴇ ʟɪɴᴋ", url=f"{message_link}"),
                             InlineKeyboardButton(
                                "ᴜɴᴘɪɴ", callback_data="pin_demote")                           
                        ]
                    ]
                ), 
                parse_mode=ParseMode.HTML,
...
def pinned(update: Update, context: CallbackContext) -> str:
    ...
    if chat.pinned_message:
        ...
        msg.reply_text(
            ...
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(text="ᴍᴇssᴀɢᴇ ʟɪɴᴋ", url=f"https://t.me/{link_chat_id}/{pinned_id}")]]
            ),
        )
...
def button(update: Update, context: CallbackContext) -> str:
    ...
    match = re.match(r"demote_nobara\((.+?)\)", query.data)
    if match:
...
@user_admin
@loggable
def promote_button(update: Update, context: CallbackContext) -> str:
    query: Optional[CallbackQuery] = update.callback_query
    user: Optional[User] = update.effective_user
    bot: Optional[Bot] = context.bot
    match = re.match(r"promote_nobara\((.+?)\)", query.data)
    if match:
        user_id = match.group(1)
        chat: Optional[Chat] = update.effective_chat
        member = chat.get_member(user_id)
        ...
        if promoted:
        	update.effective_message.edit_text(
        	    f"Admin {mention_html(user.id, user.first_name)} Promoted {mention_html(member.user.id, member.user.first_name)}!",
        	    parse_mode=ParseMode.HTML,
        	)
        	query.answer("Promoted successfully!")
        	return (
                    f"<b>{html.escape(chat.title)}:</b>\n" 
                    f"#PROMOTE\n" 
                    f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
...
DEMOTE_NOBARA_HANDLER = CallbackQueryHandler(button, pattern=r"demote_nobara", run_async=True)
PROMOTE_NOBARA_HANDLER = CallbackQueryHandler(promote_button , pattern=r"promote_nobara", run_async = True)
...
dispatcher.add_handler(DEMOTE_NOBARA_HANDLER)
dispatcher.add_handler(PROMOTE_NOBARA_HANDLER)
...
__handlers__ = [
    ...
    SET_STICKER_HANDLER,
    SETCHATPIC_HANDLER,
    DEMOTE_NOBARA_HANDLER,
    RMCHATPIC_HANDLER,
    SETCHAT_TITLE_HANDLER,
    ADMINLIST_HANDLER,
    PROMOTE_NOBARA_HANDLER,
    PIN_HANDLER,
    UNPIN_HANDLER,
    PINNED_HANDLER,
    INVITE_HANDLER,
