from Raiden.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from Raiden import SUPPORT_USERS
...
async def user_is_admin(user_id: int, message):
    ...
    async for user in telethn.iter_participants(
            message.chat_id, filter=ChannelParticipantsAdmins):
        if user_id == user.id or user_id in SUPPORT_USERS:
            status = True
            break
    return status
...
async def nobara_is_admin(chat_id: int):
    status = False
    nobara = await telethn.get_me()
    async for user in telethn.iter_participants(
            chat_id, filter=ChannelParticipantsAdmins):
        if nobara.id == user.id:
            status = True
            break
    return status
